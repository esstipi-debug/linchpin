# docs/ — design & contributor notes

Design briefs, working notes, and the image/asset library. For user-facing guides
(getting started, methodology, FAQ, capability plan), see
[`../documentation/`](../documentation/).

| Path | What it is |
|------|------------|
| [AGENT_UI_DESIGN_PROMPT.md](AGENT_UI_DESIGN_PROMPT.md) | Design prompt for the agent console UI |
| [HANDOFF.md](HANDOFF.md) | Pointer to the canonical handoff at the repo root |
| [assets/](assets/) | Screenshots and diagrams used by the README |
| [superpowers/](superpowers/) | Planning artifacts (plans, specs) |

See also the top-level [README](../README.md).
