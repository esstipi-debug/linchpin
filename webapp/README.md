...
webapp/static/operator/  Operator Portfolio page (renders documentation/operator/*.md)
...
|-------|----------------|
| `/` | the 4-tab inventory dashboard |
| `/console` | the live agent console (thin UI over `POST /api/jobs`) |
| `/warehouse` | the 3D warehouse layout (HTML) |
| `/operator` | the **Operator Portfolio** — renders `documentation/operator/*.md` (the human operator's role, runbooks, methodologies) as a navigable site; the markdown is the single source of truth, mounted read-only at `/operator-docs` and rendered client-side with a vendored `marked` (no build step). |
